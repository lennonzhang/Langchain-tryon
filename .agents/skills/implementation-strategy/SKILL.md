---
name: implementation-strategy
description: Decide how to implement runtime and API changes in langchain-tryon before editing code. Use when a task changes API endpoints, SSE contract, environment variables, provider behavior, or frontend stream/session behavior and you need to choose the compatibility boundary.
---

# Implementation Strategy

## Overview

Use this skill before editing code when the task changes runtime behavior or anything that might look like a compatibility concern. The goal is to keep implementations simple while protecting real released contracts.

## Quick start

1. Identify the surface you are changing: API endpoint, SSE event contract, environment variable, provider protocol, frontend session/stream behavior, internal backend module, or docs only.
2. Determine the latest release boundary from `origin` first, and only fall back to local tags when remote tags are unavailable:
   ```bash
   BASE_TAG="$(.agents/skills/final-release-review/scripts/find_latest_release_tag.sh origin 'v*' 2>/dev/null || git tag -l 'v*' --sort=-v:refname | head -n1)"
   echo "$BASE_TAG"
   ```
3. Judge breaking-change risk against that latest release tag, not against unreleased branch churn or post-tag changes already on `main`. If the command fell back to local tags, treat the result as potentially stale and say so.
4. Prefer the simplest implementation that satisfies the current task. Update callers, tests, and docs directly instead of preserving superseded unreleased interfaces.
5. Add a compatibility layer only when there is a concrete released consumer, a documented external contract that requires it, or when the user explicitly asks for a migration path.

## Compatibility boundary rules

- Released API endpoints, SSE event names, or documented external behavior: preserve compatibility or provide an explicit migration path.
- Environment variables, provider config, and externally consumed settings: treat as compatibility-sensitive when they are part of the latest release or when the repo explicitly intends to preserve them.
- SSE contract (event names, envelope fields, error invariants): always compatibility-sensitive. Do not silently rename SSE events.
- Interface changes introduced only on the current branch: not a compatibility target. Rewrite them directly.
- Interface changes present on `main` but added after the latest release tag: not a breaking change by themselves. Rewrite them directly unless they already define a released contract.
- Internal helpers, private modules, same-branch tests, and fixtures: update them directly instead of adding adapters.

## Default implementation stance

- Prefer deletion or replacement over aliases, overloads, shims, feature flags, and dual-write logic when the old shape is unreleased.
- Do not preserve a confusing abstraction just because it exists in the current branch diff.
- If review feedback claims a change is breaking, verify it against the latest release tag and actual external impact before accepting the feedback.
- If a change truly crosses the latest released contract boundary, call that out explicitly and update `CHANGELOG.md`.

## When to stop and confirm

- The change would alter API/SSE behavior shipped in the latest release tag.
- The change would modify environment variable semantics or defaults.
- The change would rename SSE event names or alter the error invariant.
- The user explicitly asked for backward compatibility or migration support.

## Output expectations

When this skill materially affects the implementation approach, state the decision briefly in your reasoning or handoff, for example:

- `Compatibility boundary: latest release tag v0.x.y; branch-local interface rewrite, no shim needed.`
- `Compatibility boundary: released SSE contract; preserve event names and add migration note.`
