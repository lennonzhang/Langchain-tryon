...
## Purpose
Produce the PR-ready summary required in this repository after substantive code work is complete: a concise summary plus a PR-ready title and draft description that begins with "This pull request <verb> ...". The block should be ready to paste into a PR for langchain-tryon.

## When to Trigger
- The task for this repo is finished (or ready for review) and it touched runtime code, tests, docs with behavior impact, or build/test configuration.
...
3) Infer change type from the touched paths listed under "Category signals"; classify as feature, fix, refactor, or docs-with-impact, and flag backward-compatibility risk only when the diff changes API endpoints, SSE contract, environment variables, or provider protocols. Judge that risk against `LATEST_RELEASE_TAG`, not unreleased branch-only churn.
...
git checkout -b <kebab-case suggestion, e.g., feat/tavily-search-migration>
